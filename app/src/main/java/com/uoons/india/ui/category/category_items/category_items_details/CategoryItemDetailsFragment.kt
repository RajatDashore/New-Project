...
class CategoryItemDetailsFragment :
    BaseFragment<FragmentCategoryItemDetailsBinding, CategoryItemDetailsFragmentVM>(),
    CategoryItemDetailsFragmentNavigator {
    ...
    override val viewModelClass: Class<CategoryItemDetailsFragmentVM> =
        CategoryItemDetailsFragmentVM::class.java
...
//        PID= getActivity()?.let { getEncryptedSharedprefs(it).getString("PROFILE_ID", "") }.toString()
        ...
        viewDataBinding.crdCheckPinCode.setOnClickListener(View.OnClickListener {
            if (mViewModel.isValidField(strPinCode = viewDataBinding.edtPinCode.text.toString())) {
                mViewModel.checkPinCodeAvailability(
                    viewDataBinding.edtPinCode.text.toString().toInt(), pId
                )
            }
        })
    ...
    override fun getSingleProductData() {
        if (view != null) {
            mViewModel.getSingleProductDataResponse.observe(
                viewLifecycleOwner,
                Observer<SingleProductDetailModel> {
                    if (it != null) {
                        context?.let { it2 ->
                            setDataSingleProduct(it, it2)
                        }
                    } else {
                        Toast.makeText(
                            requireActivity(),
    ...
    private fun setDataSingleProduct(data: SingleProductDetailModel, context: Context) {
        ...
        viewDataBinding.txvProductPrice.text =
            resources.getString(R.string.rupees) + data.Data?.productSalePrice
        viewDataBinding.txvMRPPrice.text =
            resources.getString(R.string.rupees) + data.Data?.productPrice
        ...
        if (discount.equals("0")) {
            ...
            viewDataBinding.txtDiscount.text =
                resources.getString(R.string.rupees) + discount + "%" + "off"
        }
        ...
        data.Data?.productDescription?.let {
            ...
            viewDataBinding.txvDescriptionDetails.text =
                Html.toHtml(HtmlCompat.fromHtml(it, HtmlCompat.FROM_HTML_MODE_LEGACY))
        }
        ...
        data.Data?.returnPolicy?.let {
            viewDataBinding.txvReturnProlicyDesicription.text =
                Html.toHtml(HtmlCompat.fromHtml(it, HtmlCompat.FROM_HTML_MODE_LEGACY))
    ...
    override fun shearLink() {
        ...
        val shareBody =
            "Download this Application now:-https://play.google.com/store/search?q=whatsapp&h1=en"
